...
export function validateSingleWorkState(authorization, activeState) {
  ...
  if (authorization?.maxOpenWorkPullRequests !== 1) errors.push('Exactly one open work PR must be allowed');
  ...
  if (activeState?.activeTask?.executionBranch && activeState.activeTask.executionBranch !== 'work') {
...
async function loadState() {
  const [authorization, activeState] = await Promise.all([
...
async function validate() {
  const { authorization, activeState } = await loadState();
  const errors = validateSingleWorkState(authorization, activeState);
  if (errors.length > 0) throw new Error(errors.join('\n'));
  console.log('Single work task state is valid.');
  return { authorization, activeState };
}
...
async function main() {
  ...
  if (command === 'validate' || command === 'preflight' || command === 'branch-check') await validate();
  else if (command === 'status') await status();
  else if (command === 'sync') await sync();
  else await rejectLegacyMutation(command);
}
