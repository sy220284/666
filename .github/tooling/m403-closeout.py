...
IMPLEMENTATION_COMMIT = '379b8518b9c2b753dd4e22392c4221be648ffb4b'
...
# Full task-card verification. Every command must pass before governance/evidence changes.
for command in [
    ['pnpm', 'lint'],
    ['pnpm', 'typecheck'],
    ['pnpm', 'build'],
...
# Task card implementation record.
...
# Traceability status reflects implementation-first governance.
...
# Contract and security documentation synchronized to the actual implementation.
...
# Format all documents before evidence hashing.
documents = [
    str(task_path),
    str(matrix_path),
    str(error_path),
...
# Four-file-plus baseline evidence manifest binds the final code implementation commit.
...
# Record implementation-first transition and activate M4-04.
run(['node', 'scripts/taskctl.mjs', 'advance', '--ci=success', f'--commit={IMPLEMENTATION_COMMIT}'])
run([
...
run(['git', 'diff', '--check'])
run(['git', 'add', '--all'])
run(['git', 'commit', '-m', '文档：收口M4-03并推进M4-04'])
