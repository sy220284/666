...
任何全量代码审计、功能审计、产品设计审计、体验审计、安全审计、恢复审计、发布审计、任务复审、PR审查或治理审查，必须同步读取并遵守 `docs/process/USER_PERSPECTIVE_AUDIT_REPORTING.md`。
...
- 审计可以在内部使用工程方法追查根因，但报告主体必须使用普通作者能够直接理解的语言。
- “竞态条件、生命周期所有权、作用域不变量、失败关闭、版本比较、原子事务、状态机漂移、请求代次、幂等、写入所有权”等工程术语不得直接充当问题标题、影响说明或审计结论。
- 工程术语确有定位价值时，只能放在用户影响之后的简短代码定位、修复附录或开发说明中。
- 每个主要问题必须说明真实触发场景、实际表现、用户损失或困惑、严重程度和正确结果；缺任一项时不得提交为合格审计结论。
- 严重程度统一按用户实际后果分为“严重 / 高 / 中 / 低”，不得用P0/P1、测试等级或内部风险编号代替。
- 严重表示数据丢失、作品损坏、无法恢复、安全问题或核心功能不可用；高表示容易得到错误内容、状态混乱或操作结果明显违背用户预期；中表示特定条件下出错、体验明显受影响或存在明确后续隐患；低表示显示、提示和操作细节问题。
- 全量审计必须从作者真实任务路径进入，同时核对产品设计、实际代码与验证证据；禁止只按目录、模块、类或工程分层罗列问题。
- 如果一个发现无法说明“用户实际会遇到什么”，不得列为主要用户问题；纯内部可维护性隐患放入独立维护项。
- 未被代码路径、设计冲突、测试或真实操作充分证明的判断必须标记为高可信风险或待核实，不得伪装成已确认问题。
- 修复建议先描述用户最终应得到的正确结果，具体工程实现留给后续修复阶段。

审计报告提交前必须自检：是否存在只有工程名词而没有用户场景的问题。存在即视为汇报不合格，必须改写。
