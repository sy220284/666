...
本文件是仓库级完整且唯一的 Agent 权威指令。`agent.md` 仅提供快速入口；冲突时以本文件为准。不得在已批准文档之外擅自扩展产品范围、架构、依赖、云服务或任务顺序。
...
- 新建及活动Runtime使用 `executionBranch: "work"`；已Verified Runtime中的历史来源分支保持冻结，不得为表面统一篡改。
- `ACTIVE_TASK.json` 与`ACTIVE_TASK.md`只作为旧串行状态机和历史关闭流程的兼容锚点，必须同步，但不得覆盖Schema 2授权真源。
...
- 所有正式PR必须精确为 `work → main`，且来源仓库必须是当前仓库。
...
- 不允许验证专用分支、治理专用分支、纯Evidence分支或纯关闭PR。
...
Squash后`work`与`main`提交身份不同，因此同步动作统一称为“受控重置”，不得称为普通快进。
...
- `Implemented`只证明实现已进入受检PR Head，不能冒充Verified、阶段关闭或发布资格。
- Main Verification成功后发布 `main-verification`及任务验证状态；不再创建第二个关闭PR。
- Release Gate必须读取有效状态，不能只相信Runtime中的静态文字。
- 最终Evidence在合并前记录来源PR和受检Head；合并后的main SHA、验证运行和状态由GitHub提交状态提供权威闭环。
