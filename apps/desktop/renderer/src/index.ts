...
const openTextIoButton = document.querySelector<HTMLButtonElement>('[data-legacy-open-text-io]');
const openPlanningButton = document.querySelector<HTMLButtonElement>('[data-legacy-open-planning]');
...
async function refreshProjectStructure(): Promise<void> {
  const generation = ++structureRefreshGeneration;
  const project = activeProject;
  if (!project) {
  ...
  try {
    ...
    if (generation !== structureRefreshGeneration || activeProject?.projectId !== project.projectId)
      return;
    if (result.ok) {
      renderProjectStructure(result.data);
      setStructureState(
...
function legacyAction(label: string, attribute: string): HTMLButtonElement {
  const button = document.createElement('button');
  button.type = 'button';
  button.className = 'quiet-button';
  button.textContent = label;
...
function renderTrashEntries(entries: readonly TrashEntry[]): void {
  ...
  for (const entry of entries) {
    ...
    const restoreOriginal = legacyAction('恢复原位', 'data-restore-original');
    const restoreEnd = legacyAction('恢复到末尾', 'data-restore-end');
    const restoreElsewhere = legacyAction('恢复到其他卷', 'data-restore-elsewhere');
    const permanentDelete = legacyAction('永久删除', 'data-permanent-delete');
    const readOnly = activeProject?.databaseMode !== 'read-write';
...
const openRecoveryButton = document.querySelector<HTMLButtonElement>('[data-legacy-open-recovery]');
