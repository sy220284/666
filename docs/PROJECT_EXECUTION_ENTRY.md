...
`ACTIVE_TASK.json`是机器可读真源，`ACTIVE_TASK.md`是其镜像。当前处于作者预授权的`continuous-mainline`模式：前一任务Verified且证据、追踪和依赖门完整后，可自动激活下一任务；任一门失败立即阻断。
...
- 连续主线模式只能在当前任务Verified后自动推进，禁止并行任务、跳过失败或越过阶段门。
