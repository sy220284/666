...
`TASK_AUTHORIZATION.json`定义唯一`work`分支、任务PR与main串行写入规则；`docs/tasks/runtime/`保存任务机器状态和历史验证记录。M9、M10-01及M10-02均已完成Verified闭环；当前活动任务为M10-03。
...
- 当前`work`包含尚未合并的M10-03修改；在Main Verification成功前，不得把任一M10-03 work提交称为已验证产品基线。
...
- 历史Migration、数据库Schema、IPC Channel字符串、`PROTOCOL_VERSION`、正式错误码和公开Bridge方法保持不变。
