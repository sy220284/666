...
| 错误码 | 含义 |
|---|---|
| `DRAFT_REVISION_CONFLICT_001` | 保存序号与当前保存序号不一致 |
| `DRAFT_BLOCK_HASH_CONFLICT_002` | 目标正文块内容已变化 |
| `DRAFT_BLOCK_LOCKED_003` | 操作涉及锁定正文块；`details.lockConflict`列出安全冲突摘要 |
| `DRAFT_PATCH_INVALID_004` | 正文补丁结构或顺序无效 |
| `DRAFT_NO_ACTIVE_005` | 章节没有活动当前稿 |
| `VERSION_IMMUTABLE_001` | 尝试修改不可变历史版本 |
| `VERSION_CREATE_FAILED_002` | 历史版本创建事务失败 |
| `CANDIDATE_ALREADY_RESOLVED_001` | 建议稿已采用或丢弃 |
| `CANDIDATE_BASE_CONFLICT_002` | 建议稿基线已过期 |
| `CANDIDATE_PARTIAL_RESTRICTED_003` | 未完成建议稿不能执行该操作 |

## 6. AI与Provider

| 错误码 | 含义 | 可重试 |
|---|---|---:|
| `AI_PROVIDER_NOT_CONFIGURED_001` | AI连接未配置 | 否 |
| `AI_CREDENTIAL_MISSING_002` | 系统凭据不可用 | 否 |
| `AI_CONNECTION_FAILED_003` | 无法连接模型服务 | 是 |
| `AI_AUTH_FAILED_004` | 认证失败 | 否 |
| `AI_RATE_LIMITED_005` | Provider限流 | 是 |
| `AI_REQUEST_TIMEOUT_006` | 模型请求超时 | 是 |
| `AI_CONTEXT_OVERFLOW_007` | 发送上下文超过模型预算 | 否，需裁剪 |
| `AI_OUTPUT_INVALID_008` | 返回内容无法通过目标Schema或业务输出校验 | 视任务而定 |
| `AI_STREAM_INTERRUPTED_009` | 流式连接中断 | 是，可保存未完成建议稿 |
...
| `EXPORT_VERSION_REQUIRED_001` | 未选择可导出的历史版本 |
...
| 错误码 | 含义 |
|---|---|
| `SEARCH_INDEX_UNAVAILABLE_001` | 全文搜索索引不可用，可重建 |
| `SEARCH_REPLACE_PLAN_STALE_002` | 替换预览已过期 |
| `VALIDATION_INPUT_STALE_001` | 作品检查对应历史版本已变化 |
...
- 保存序号和内容校验冲突：打开冲突处理，不显示为普通Toast。
- 锁定冲突：显示安全的冲突块与整批跳过摘要；不得暗示未冲突操作已写入。
- AI响应超限：说明系统已停止接收且正文未修改，提供缩小范围或检查服务动作。
- 用户取消：不显示红色错误。
...
1. 在`packages/contracts`登记并通过严格Schema。
2. 增加至少一个失败路径测试。
3. 定义Renderer行为和是否可重试。
4. 同步对应协议、安全、测试和作者提示文档。
5. 不复用已发布错误码表达新语义。
