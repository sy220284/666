...
| CommandReceipt   | 与业务事务原子提交的持久命令回执，用于声明支持的命令在Core崩溃或重启后重放首次结果 |
| SemanticRevision | 项目权威语义图的单调递增修订号，由数据库Trigger维护，供Validation判断语义新鲜度    |
...
节点可依赖同项目ArcMilestone或TimelineEvent。命中ArcMilestone前，所有前置ArcMilestone必须已经`hit`；TimelineEvent没有`hit`状态，其依赖满足条件为事件保持`active`、存在有效章节锚点，且锚点章节在作品权威顺序中不晚于本次实际命中章节。作者直接transition与StateProposal接受复用同一依赖策略，分别记录`confirmationSource=author`或`state_proposal`。AI不能创建、修改或推进伏笔、人物弧光和弧光节点权威状态；`pending`弧光提案不能提前改变ArcMilestone状态。
...
| TaskSnapshot        | TaskProtocol提供的运行阶段、进度和事件投影，不是生成业务生命周期真源                   |
| requestId           | 命令身份与幂等键，不等同于Run ID；是否支持跨Core重启重放必须由具体命令及其持久记录声明 |
...
| semantic freshness  | Validation读取`SemanticRevision`并结合章节内容digest判断缓存或批次是否仍对应当前权威语义 |
