...
| ID      | 功能           | 设计与效果                                                               | 主要关系                               | 版本     |
| ------- | -------------- | ------------------------------------------------------------------------ | -------------------------------------- | -------- |
| AEX-001 | 正式中文名称   | 术语表为唯一业务语言真源，界面与协作文本受永久检查                       | 全部作者界面、开发门禁                 | V1.0维护 |
| AEX-002 | 精准跳转与返回 | 保存来源页面、选择、筛选、滚动和焦点；目标失效时不误跳                   | 搜索、作品检查、待办、伏笔、场景节拍   | V1.0维护 |
| AEX-003 | 写作辅助聚合   | 只读汇总章节目标、场景节拍、人物状态、伏笔、待办与上一章结尾             | 规划、设定、作品检查、历史版本         | V1.0维护 |
| AEX-004 | 结构化设定编辑 | 人物、地点、章节、定稿版本、事件和伏笔使用名称选择与常用字段             | 设定、连续性、叙事规划                 | V1.0维护 |
| AEX-005 | 建议稿差异审阅 | 分组、并排与行内差异、只看修改、长章节折叠和安全采用                     | 建议稿、历史版本、正文块补丁           | V1.0维护 |
| AEX-006 | 安全关闭握手   | 主进程、预加载桥和应用界面通过具名合同刷新当前稿；失败或超时阻止静默退出 | 当前稿、本地服务生命周期、进程通信合同 | V1.0维护 |
| AEX-007 | 异步查询代次   | 后发请求与作品切换使旧响应失效，避免跨作品或同作品回写旧结果             | 全文搜索、写作辅助                     | V1.0维护 |
