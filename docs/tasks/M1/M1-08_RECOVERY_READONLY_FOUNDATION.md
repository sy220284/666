...
1. 当`project.sqlite`物理损坏到SQLite无法打开时，仍以`manifest.json`建立只恢复上下文。
2. 独立“恢复损坏项目”入口只允许读取外部恢复点；Draft、Version、结构和全部写命令保持阻断。
3. 恢复始终写入新目录和新项目ID，不覆盖、修复或截断损坏源文件。
4. 增加真实SQLite头损坏的Integration与Electron E2E，并留存可见桌面验收截图。
