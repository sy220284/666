...
修复M9全量审计发现的章节切换编辑器锁死问题，加固Task事件序号缺口恢复，并以可执行测试收窄Renderer异步生命周期盲区。
...
4. 增加章节状态策略、源码不变量和Task恢复交错测试。
5. 通过新增全覆盖纯逻辑模块提高分支覆盖余量；不降低现有75%门槛，不扩大覆盖排除。
...
按本任务提交整体回退章节状态策略、Task恢复协调器及专项测试。回退不得单独删除测试而保留异步行为变更。
