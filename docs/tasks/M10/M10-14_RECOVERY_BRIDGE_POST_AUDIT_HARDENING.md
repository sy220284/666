...
根据 M10-13 合并后的全量代码 Review，修复剩余 Recovery fail-closed、每日备份并发、Renderer Bridge 共享读取取消语义、Provider IPv6 特殊地址分类、高风险生命周期覆盖盲区，以及最终桌面验证暴露的 Planning 状态完成顺序竞态，保持已验证的数据、Migration、Generation 与安全内核不变。
...
7. PlotNode 保存成功后先关闭弹窗、再等待 Outline 刷新、最后写“已保存”，旧操作可能在后续移动完成后晚写状态，覆盖更新的“已移动”反馈。
...
- Planning 操作完成状态不得在异步刷新结束后晚写并覆盖更新操作的反馈；
...
- Planning PlotNode 保存成功状态改为在等待 Outline 刷新前同步提交，原 `project-planning` E2E 保持原断言即从失败恢复为通过；
- Ready 验证过程中发现的 Recovery 错误包装、同步抛错与 Planning 状态晚写兼容问题均已按既有公共契约修复。

## 永久验证

最终实施提交 `ed06fe1317f3c23d67c06cb4469852829ff34c3a` 已通过：

- Quality run `31148118685`：Static、Unit、Integration、Migration、Coverage、Build、Electron E2E、聚合 Quality 全部成功，Electron E2E 33/33；
