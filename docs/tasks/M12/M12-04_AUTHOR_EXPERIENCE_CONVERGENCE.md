...
- [x] 规划模式只有一个状态真源。
- [x] 研究资料筛选与编辑上下文完全分离。
- [x] Feature / 普通 Runtime 的浏览器原生 `prompt/confirm` 已迁移到单一应用内作者弹窗；关闭握手例外明确且受测。
- [x] 三条创作路径真实影响首页入口与推荐，但不分叉底层业务。
- [x] AI与 Candidate 简明模式不再暴露无必要技术标识，完整模式仍可诊断。
- [x] 内容检查工作台完成页内分区且状态保持。
- [x] 异步任务刷新没有新增高频重复轮询。
- [x] 8K / 20K 单章性能基线完成；无证据不进行过度性能重构。
- [x] Runtime 指定验证真实通过，Ready Evidence 绑定最新实现提交。
