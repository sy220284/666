...
- 目标：覆盖长篇最常见的时间、信息差、伏笔和人物弧光连续性。
- 依赖：M2-02。
- 分支：`feat/m2-continuity-models`
- 关联：REQ-019—021、REQ-045，P0-038—040、P0-071、072。
...
- 页面与章节/SceneBeat关联。
- CharacterArc（成长/黑化/觉醒/堕落/救赎类型）与ArcMilestone状态机（planned/hit/skipped），弧光命中经StateProposal确认（见ADR-006）。
- 弧光当前阶段与EntityState性格标签的一致性校验钩子（供M4-01 VAL-003使用）。
...
时间精度不同时的冲突规则、知情状态变化、伏笔状态迁移、依赖循环提示、软删除引用、弧光节点状态机流转、弧光命中前不改变权威状态。
