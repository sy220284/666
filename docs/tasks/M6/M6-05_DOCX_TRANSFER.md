...
补齐校验、全项目搜索、节奏指标、DOCX和三轨备份恢复。本任务扩展既有导入导出路径，不建立第二套ImportPlan、提交协调器或恢复事务，并接入M6-04统一`mutationOrigin`合同。
...
- M6-04统一`mutationOrigin: import`合同。
...
3. 限制解包总大小、单文件大小、文件数、压缩比、嵌套深度和路径；具体上限集中配置并有边界Fixture，禁止散落魔法数字。
4. 忽略宏、OLE、脚本、外部模板和远程资源；使用隔离临时目录和安全ZIP读取。
5. 路径穿越、符号链接、设备文件、异常关系、重复条目和大小声明不一致必须拒绝。
6. 失败、取消、超时和进程异常时清理临时内容、计划和未提交资源。
7. 提交继续复用计划过期、路径重校验、恢复点和单事务写入；任务开始后原文件或项目状态变化时计划必须失效。
8. DOCX导入产生的全部Draft变更必须由Core标记`mutationOrigin: import`，Renderer不得提供或覆盖来源；供M6-04排除人工写作统计。
9. 从作者选定的不可变Version导出TXT、Markdown和DOCX，禁止读取Candidate、当前Renderer HTML或未确认临时流。
10. 输出使用同目录临时文件、内容验证、fsync/关闭和原子重命名；目标冲突由明确策略处理。
11. DOCX适配器只负责格式转换和安全解析，不复制项目事务、恢复、Version选择或写作统计逻辑。

## 测试与证据

- 异常DOCX、ZIP路径穿越、压缩炸弹、超限文件、嵌套关系、宏、OLE、外部资源、重复条目和空内容。
...
- DOCX与既有TXT/Markdown导入全部标记为`import`；人工净增和写作速度不变化。
- Renderer伪造或覆盖`mutationOrigin`被拒绝。
...
- DOCX与既有文本导入统一使用Core决定的`mutationOrigin: import`。
