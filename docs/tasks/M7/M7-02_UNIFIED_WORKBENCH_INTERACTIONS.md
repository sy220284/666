...
5. StatusArbiter只组合Core和正式Use Case真实状态，不以Renderer推测或伪造进度。
6. 将AI任务、Candidate待审、StateProposal待确认、ValidationIssue、索引stale、备份失败和只读风险纳入统一状态分级。
7. 上下文帮助采用控件提示、首次提示和页面短帮助三层；技术诊断与普通作者帮助明确分层。
8. 覆盖空、加载、成功、失败、取消、冲突、只读、过期计划、恢复和结果未知状态。
9. 未实现、不可用或受限功能不得显示为正常可点击入口；禁用原因必须具体。
10. 沉浸视图继续复用M5-00状态，只控制视觉披露，不创建第三套产品模式。
11. 全局键盘导航、焦点返回、窗口关闭重启和后台任务恢复在各工作台一致。
12. 删除或合并全功能接入后形成的重复入口和重复状态提示，但不得删除真实恢复路径。
...
任务关闭前必须同步`TASK_INDEX.md`、`V1.0_TRACEABILITY_MATRIX.md`及实际受影响的Schema、IPC、UI或测试文档。
