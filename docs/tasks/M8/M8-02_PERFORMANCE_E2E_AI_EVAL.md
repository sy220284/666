...
6. 单独验证T0绕过：无Skeleton、无SceneBeat时以直接章节目标生成Prose Candidate。
7. 验证Skeleton结构化Payload持久化、作者编辑、T1按ID读取和禁止正文Apply。
8. 验证partial保存、继续生成、手动补全、整稿采用拒绝和直接定稿拒绝。
9. 验证GenerationRun在切章、切页、取消、断流和应用重启后的真实状态，不宣称内存流已恢复。
10. 验证Final Version→`state_extract`→pending StateProposal→作者接受/编辑接受/拒绝→权威状态与快照完整闭环。
11. pending提案不得参与M6-02权威语义校验，作者确认后状态必须正确参与。
12. 按Provider、Model、Task、PromptVersion记录T0、T1、rewrite、merge、validate、state_extract和连续性Eval。
13. Prompt、Eval和ModelSupportProfile版本必须一致；不一致时支持等级降级或阻断。
14. 验证M6-04人工写作统计：AI采用、导入、批量替换、恢复、结构操作和系统维护均不得计入人工净增或有效写作速度。
15. 完成1280×800、2K、21:9、混合DPI、两套主题、减少动态、键盘和读屏矩阵。
16. 所有性能与体验结果记录真实数据、设备环境、Fixture规模和统计方法，禁止伪造倒计时或进度。

## 测试与证据

- 性能、E2E、Eval、显示、无障碍和统计口径矩阵全部运行。
- T0三种输入路径、Skeleton结构化往返和Apply拒绝。
- 取消后无迟到delta、断流和重启真实状态。
- partial限制与继续生成。
- T0→T1→Candidate→采用→定稿全链路。
- 定稿→state_extract→StateProposal→作者确认→快照全链路。
- pending隔离和确认后状态参与校验。
- 人工写作统计排除全部非人工来源。
