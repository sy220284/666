...
> 前置基线：`main@4c6ccc51707a377b8589826d443a294dc8fd1385`  
> 前置任务：`M8-07`完成实现、验证、main受控合并与最终验证闭环  
> 后续任务：无  
> 目标终态：`VERIFIED_HOLD`

## 一、任务定位

本任务承接M8-07完成后的最新`main`，对WorldForge V1.0进行一次性最终质量治理与封版闭环。
...
禁止将正文保存竞态、AI检查轮询、异常状态、能力矩阵、测试治理、版本升级或封版文档拆成新的独立任务。

## 二、执行前置条件

M8-08是M8-07的后继任务，不得与M8-07并行实施。

只有同时满足以下条件，才允许将M8-08从`Planned`激活为`In Progress`：

1. M8-07实现PR已受控合并到`main`。
2. M8-07永久门禁全部成功。
3. M8-07最终main验证完成。
4. M8-07 Evidence绑定最终提交。
5. M8-07任务状态已转为Verified或仓库规定的等价终态。
6. M8-08基线已重新更新为届时最新`main`提交。
7. `ACTIVE_TASK.json`、`PROJECT_EXECUTION_ENTRY.md`和`TASK_INDEX.md`完成一致激活。

本任务卡写入`main`只代表预先登记后继治理范围，不代表M8-08已经激活，不得改变当前M8-07唯一活动任务状态。
...
旧保存结果返回时，不得使用旧内容覆盖当前编辑器中的新状态。

#### 1.2 正文块元数据不得按位置错误绑定

禁止仅按照数组位置将服务端返回的以下元数据绑定到当前编辑器正文块：
...
- 当前编辑器已继续编辑但结构未变化时，只同步能够通过稳定身份确认的元数据。
- 不得覆盖当前文本、选区、编辑历史和保存后新增修改。
- 当前编辑器结构已变化时，禁止使用旧结果执行正文重置。
- 当前状态必须继续保持未保存标记，并根据最新状态调度下一轮保存。
...
打包冒烟测试不得只以`rendererReady=true`认定产品可用，至少验证：
...
只执行静态检查时，禁止发布“Final main SHA passed full Linux verification”等完整验证描述。

状态文案必须与实际执行范围一致。只有最终main完整运行验证成功后，才允许发布完整验证通过状态。

### 4. 合并一致性

来源PR Head必须同步最新main。Squash Merge完成后，必须对最终main完整执行运行门禁。本任务以最终main完整复跑为强制要求。
...
禁止回写或改造已经冻结的历史任务实现Evidence。

允许：

- 修正当前汇总状态。
- 在M8-08 Evidence中引用历史任务。
...
除非确认无法在不修改Schema的情况下修复P0问题，否则禁止新增或修改数据库Migration。

确需Schema变化时，必须在本任务内完成，并提供：
...
14. M8-08最终建立`VERIFIED_HOLD`，`deferredVerification`和`deferredTasks`均为空，`nextTaskId=null`。
