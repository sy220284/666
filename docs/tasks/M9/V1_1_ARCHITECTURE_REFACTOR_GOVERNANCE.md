...
本治理PR只冻结方案，不自动解除当前V1.0 `VERIFIED_HOLD`。开始实施前必须另行执行任务激活治理：

1. 将AR工作包转换为正式机器任务卡和Runtime状态。
2. 在`TASK_INDEX.md`登记任务与依赖。
3. 将`verificationHold.finalTask`调整为`false`，并设置下一任务。
4. 保留M8-09作为最后一个V1.0 Verified锚点。
5. 首先只激活AR-01，其他工作包保持Planned。

未完成上述激活前，任何生产代码拆分均不属于本方案授权范围。
