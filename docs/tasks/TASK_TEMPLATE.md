...
> 本模板字段为完整口径。`docs/tasks/M0`–`M5`下的具体任务卡为精简版，只保留目标/依赖/必读文档/实施内容/测试/完成条件。精简版缺失的“影响范围、数据库变化、IPC变化、失败与冲突路径、回滚策略”等板块，由执行者按`docs/PROJECT_EXECUTION_ENTRY.md`§5.1的任务启动流程在开工前现场生成，并写入完成报告，不要求预先补全到任务卡文件里。
