...
- Core读取锚点时重新核验活动章节、活动Draft、逻辑块Hash和光标范围；失效位置返回安全回退状态。
...
`project_settings.value_json`只包含定位元数据。正文、Prompt、凭据、原始Provider响应和用户选择文本均不会进入继续写作设置。锚点与活动项目ID不一致时返回空结果；锚点指向已变化正文块时返回`stale`，Renderer打开首个可用章节。
...
- Electron Playwright：当前容器缺少X Server，进程在Renderer启动前退出；该项交由PR CI的显示环境复核。
