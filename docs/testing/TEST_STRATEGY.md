...
Theme A/B切换→候选采用→定稿→减少动态→QHD/QHD+/UWQHD/4K/混合DPI→业务结果、选区和未保存文本不变。
...
M11-07额外使用真实300万字与500万字项目，记录项目重开、全文索引、命令面板查询路径、故事知识窗口、约束包、三级摘要全量/增量重建、自动保存、历史版本、SQLite体积、堆增长，以及500万字已验证备份和恢复副本。生成和导航路径必须只读取有界摘要、结构或FTS结果，不得逐次扫描全书正文。
...
- 节奏提示范围、关闭行为和P3等级。
- 中文模型废话。
- 语义检索证据门分别测量FTS精确召回、卷章结构跳转和跨卷摘要上下文覆盖；只有关键作者场景覆盖低于门槛时才允许后续评估向量层。未触发时明确记录“不新增向量或Embedding表”。
