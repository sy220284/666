...
export interface ImportExportServiceOptions {
  ...
  readonly faultInjector?: (stage: 'after-checkpoint' | 'during-import' | 'after-export-write') => void;
}
...
function parseMarkdown(text: string, fallbackTitle: string, idFactory: () => string): ImportPlanChapter[] {
...
function parseTxt(text: string, fallbackTitle: string, idFactory: () => string): ImportPlanChapter[] {
...
async function existingWritableDirectory(directory: string): Promise<string> {
  ...
  if (details.isSymbolicLink() || !details.isDirectory()) {
    throw new ImportExportServiceError('EXPORT_WRITE_FAILED', 'The export target is not a directory.');
  }
...
export class ImportExportService {
  ...
  readonly #faultInjector:
    | ((stage: 'after-checkpoint' | 'during-import' | 'after-export-write') => void)
    | undefined;
  ...
  constructor(
    ...
    this.#writeTarget = options.writeTarget ?? ((filePath, content) => writeFile(filePath, content));
  ...
  async previewImport(raw: ImportPreviewInput, selectedPath: string): Promise<ImportPlan> {
    ...
    const encoding = input.encoding && input.encoding !== 'auto' ? input.encoding : detected.encoding;
    const text = decode(buffer, encoding);
    if (!text.replace(/[\s\uFEFF]/gu, '')) {
      throw new ImportExportServiceError('IMPORT_CONTENT_EMPTY', 'The selected document has no text.');
    }
    if (text.includes('\u0000')) {
      throw new ImportExportServiceError(
        'IMPORT_FORMAT_UNSUPPORTED',
    ...
    if (chapters.length === 0) {
      throw new ImportExportServiceError('IMPORT_CONTENT_EMPTY', 'No importable chapter was found.');
    }
    ...
    try {
      await this.#workspace.writeProject(requestId, input.projectId, (database) => {
        ...
        database
          ...
          .run(volumeId, input.projectId, input.volumeTitle, BigInt(currentOrder.orderKey) + ORDER_STEP);
