...
type ProjectDatabase = Parameters<
  Parameters<ProjectWorkspaceService["readProject"]>[1]
>[0];
...
function parseTags(value: string): string[] {
  ...
  throw new ResearchServiceError(
    "RESEARCH_INVALID",
    "Stored research tags are invalid.",
  );
}
...
function sourceExists(
  ...
  const table =
    sourceType === "note" ? "research_notes" : "research_attachments";
  return (
    database
...
function targetExists(
  ...
  const queries: Record<ResearchTargetType, string> = {
    ...
    relationship:
      "SELECT 1 FROM character_relationships WHERE id = ? AND project_id = ?",
    timeline: "SELECT 1 FROM timeline_events WHERE id = ? AND project_id = ?",
    foreshadowing:
      "SELECT 1 FROM foreshadowings WHERE id = ? AND project_id = ?",
    arc: "SELECT 1 FROM character_arcs WHERE id = ? AND project_id = ?",
    idea: "SELECT 1 FROM idea_cards WHERE id = ? AND project_id = ?",
  };
  return (
    database.prepare(queries[targetType]).get(targetId, projectId) !== undefined
  );
}
...
function refreshResearchFts(
  ...
  database
    .prepare("DELETE FROM fts_research_notes WHERE note_id = ?")
    .run(noteId);
...
export class ResearchService {
  ...
  async importAttachment(
    ...
    try {
      ...
      if (
        ...
        copied.size > MAX_ATTACHMENT_BYTES
      ) {
        throw new ResearchServiceError(
          "RESEARCH_ATTACHMENT_FAILED",
      ...
      return await this.#workspace.writeProject(
        requestId,
        input.projectId,
        (database) => {
          database
      ...
      const catalog = await this.#workspace.writeProject(
        ...
        (database) => {
          ...
          const deleted = database
            .prepare(
              "DELETE FROM research_attachments WHERE id = ? AND project_id = ?",
            )
    ...
    return this.#workspace.writeProject(
      ...
      (database) => {
        ...
        if (
          ...
          throw new ResearchServiceError(
            "RESEARCH_NOT_FOUND",
            "Research link target not found.",
          );
        }
        database
          .prepare(
  ...
  #noteExists(projectId: string, noteId: string): boolean {
    return this.#workspace.readProject(
      ...
      (database) =>
        database
          .prepare(
            "SELECT 1 FROM research_notes WHERE id = ? AND project_id = ?",
          )
  ...
  #throwWriteConflict(
    ...
    throw new ResearchServiceError(
      ...
      exists
        ? "The research note changed before this update."
        : "Research note not found.",
    );
  }
  ...
  #catalog(database: ProjectDatabase, raw: ResearchListInput): ResearchCatalog {
    ...
    const attachments = sqliteResult<Record<string, unknown>[]>(
      ...
      .filter(
        (attachment) =>
          attachment.noteId === null || noteIds.has(attachment.noteId),
      );
