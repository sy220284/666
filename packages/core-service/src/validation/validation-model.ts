...
function semanticRevision(database: DatabaseSync, projectId: string): number | bigint {
  const row = database
    .prepare('SELECT revision FROM semantic_revision WHERE project_id = ?')
    .get(projectId) as { readonly revision: number | bigint } | undefined;
  if (!row) {
    throw new ValidationServiceError(
      'VALIDATION_INVALID',
      'The project semantic revision authority is unavailable.',
    );
  }
  return row.revision;
}
...
export function semanticInvalidationDigest(
  ...
  return hash(
    stableJson({ projectId, chapterId, semanticRevision: semanticRevision(database, projectId) }),
  );
}

export function authoritativeSemanticDigest(database: DatabaseSync, projectId: string): string {
  return hash(stableJson({ projectId, semanticRevision: semanticRevision(database, projectId) }));
}
...
export function validateScopedIds(
  ...
  if (input.chapterId && !isActiveChapter(database, input.projectId, input.chapterId)) {
    throw new ValidationServiceError('VALIDATION_NOT_FOUND', 'Active chapter not found.');
  }
    ...
    const beat = database
      .prepare(
        `SELECT 1
           ...
           JOIN volumes volume ON volume.id = chapter.volume_id
          WHERE beat.id = ? AND beat.project_id = ? AND beat.deleted_at IS NULL
            AND chapter.deleted_at IS NULL AND volume.deleted_at IS NULL`,
      )
