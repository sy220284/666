...
# 1. GitHub Release workflow: always unsigned, no signing secrets or notarization steps.
...
text = replace_once(
    ...
    "",
    "remove distribution trust input",
)
text = replace_once(
    text,
...
# 2. Release acceptance policy: stable can be allow-unsigned.
...
text = replace_once(
    ...
    "forbid signing credentials in release workflow",
)
save(path, text)

# 3. Packager and verifier default stable to unsigned; explicit required mode remains available below the release workflow.
...
text = replace_once(
    ...
    "",
    "remove package stable hard gate",
)
save(path, text)
...
text = replace_once(
    ...
    "",
    "remove verifier stable hard gate",
)
save(path, text)
...
# 4. Unit tests lock the new policy while retaining explicit required-mode trust verification tests.
...
text = replace_once(
    ...
    "stable publish acceptance test",
)
text = replace_once(
    text,
    ...
    "signing credential regression test",
)
save(path, text)
...
text = replace_once(
    ...
    "",
    "remove stable unsigned rejection",
)
text = replace_once(
    text,
    ...
    "add stable unsigned package test",
)
save(path, text)

# 5. Active documentation follows the author's explicit self-use policy.
save(
