...
export function mainVerificationDispatchBody(config, mergeSha, number, sourceHeadSha) {
  if (!/^[0-9a-f]{40}$/iu.test(mergeSha ?? '')) throw new Error('Controlled merge did not return a full commit SHA');
  if (!Number.isSafeInteger(number) || number <= 0) throw new Error('Main verification requires a valid pull request number');
  if (!/^[0-9a-f]{40}$/iu.test(sourceHeadSha ?? '')) throw new Error('Main verification requires the checked pull request head SHA');
  if (!config?.baseBranch || !config?.mainVerificationWorkflow) throw new Error('Main verification workflow configuration is missing');
  return { ref: config.baseBranch, inputs: { expected_sha: mergeSha, source_pr: String(number), source_head_sha: sourceHeadSha } };
}

async function hasMainVerificationRun(owner, repo, workflow, sha) {
  const encodedWorkflow = encodeURIComponent(workflow);
  const response = await api(`/repos/${owner}/${repo}/actions/workflows/${encodedWorkflow}/runs?event=workflow_dispatch&head_sha=${sha}&per_page=20`);
...
async function main() {
  ...
  if (event.workflow_run?.name !== 'Quality') throw new Error('Auto Merge must be triggered only by the Quality workflow');
  ...
  for (const item of pulls) {
    ...
    const merged = await api(`/repos/${owner}/${repo}/pulls/${number}/merge`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sha, merge_method: config.mergeMethod, commit_title: `${pull.title} (#${number})` }),
    });
