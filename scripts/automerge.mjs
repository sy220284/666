...
async function signals(token, owner, repo, sha) {
  const [checks, statuses] = await Promise.all([
    pages(token, `/repos/${owner}/${repo}/commits/${sha}/check-runs?per_page=100`, 'check_runs'),
    pages(token, `/repos/${owner}/${repo}/commits/${sha}/statuses?per_page=100`),
  ]);
...
async function blockedByReview(token, owner, repo, number, config) {
  if (config.blockChangesRequested) {
    ...
    if ([...latestReviewStates(reviews).values()].includes('CHANGES_REQUESTED')) return true;
  }
  return config.blockUnresolvedThreads && (await hasUnresolvedThreads(token, owner, repo, number));
}
...
export function mainVerificationDispatchBody(config, mergeSha, number, sourceHeadSha) {
  ...
  if (!/^[0-9a-f]{40}$/iu.test(sourceHeadSha ?? '')) throw new Error('Invalid source head SHA');
  return {
...
export async function ensureMainVerification(
  ...
  const main = await api(token, `/repos/${owner}/${repo}/git/ref/heads/${config.baseBranch}`);
  if (main.object.sha !== mergeSha) return;
  if (await hasVerification(token, owner, repo, config.mainVerificationWorkflow, mergeSha)) return;
  await api(
    token,
...
async function main() {
  ...
  if (pulls.length === 0)
    pulls = await pages(token, `/repos/${owner}/${repo}/commits/${sha}/pulls?per_page=100`);
  ...
  for (const item of pulls) {
    ...
    if (
      ...
      continue;
    if (pull.merged) {
      await ensureMainVerification(owner, repo, config, pull.merge_commit_sha, number, sha, token);
      continue;
    }
    if (pull.state !== 'open' || (config.blockDrafts && pull.draft)) continue;

    const mainRef = await api(token, `/repos/${owner}/${repo}/git/ref/heads/${config.baseBranch}`);
    const comparison = await api(
      token,
      `/repos/${owner}/${repo}/compare/${mainRef.object.sha}...${sha}`,
    );
    if (comparison.behind_by > 0) continue;
    const checkState = await waitChecks(token, owner, repo, sha, config.requiredChecks);
    ...
    if (refreshed.behind_by > 0 || (await blockedByReview(token, owner, repo, number, config)))
      continue;
