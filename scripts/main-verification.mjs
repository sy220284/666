...
export function validateMainVerification({
  ...
  if (githubSha !== expectedSha) {
    throw new Error(
      `Dispatched SHA ${githubSha} does not match expected main SHA ${expectedSha}`,
    );
  }
  if (!pull?.merged || !pull.merged_at)
    throw new Error(
      `Pull request #${sourcePr} is not merged`,
    );
  ...
  if (pull.head?.sha !== sourceHeadSha) {
    throw new Error(
      `Pull request #${sourcePr} head SHA changed after permanent checks`,
    );
  }
  if (pull.merge_commit_sha !== expectedSha) {
    throw new Error(
      `Pull request #${sourcePr} did not produce ${expectedSha}`,
    );
  }
  ...
  if (!state.ready) {
    ...
    throw new Error(
      `Source PR permanent checks are not successful: ${details.join(", ")}`,
    );
  }
}
...
export async function waitForSourceReadyChecks({
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    if (state.failed.length > 0) {
      throw new Error(
        `Source PR permanent checks failed: ${state.failed.join(", ")}`,
      );
    }
    ...
    if (attempt === 1 || attempt % 6 === 0) {
      log(
        `Witing for source PR permanent checks: ${state.pending.join(", ")}`,
      );
    }
    await sleep(delayMs);
  }
...
async function paginatedCollection(token, pathname, collectionKey = null) {
  ...
  while (next) {
    ...
    if (!Array.isArray(page)) {
      throw new Error(
        `GitHub API pagination payload is missing ${collectionKey ?? "array data"}`,
      );
    }
    items.push(...page);
...
async function publishStatus() {
  ...
  assertFullSha(sourceHeadSha, "SOURCE_HEAD_SHA");
  if (!Number.isSafeInteger(sourcePr) || sourcePr <= 0)
    throw new Error("SOURCE_PR is invalid");
