...
const file = 'scripts/taskctl.mjs';
let source = await readFile(file, 'utf8');

if (source.includes('writeTaskStateTransaction')) {
  console.log('taskctl atomic migration already applied.');
  process.exit(0);
}
...
source = source.replace(
  ...
  "import { validateAllVerifiedEvidence } from './verified-evidence-scan.mjs';\nimport {\n  recoverAtomicFileTransactions,\n  writeFilesAtomically,\n} from './atomic-file-transaction.mjs';",
);

const oldLoad = String.raw`async function load() {
...
const newLoad = String.raw`async function load() {
  const [stateSource, indexSource, mirrorSource] = await Promise.all([
    readFile(statePath, 'utf8'),
    readFile(indexPath, 'utf8'),
    readFile(mirrorPath, 'utf8'),
...
if (!source.includes(oldLoad)) throw new Error('load() baseline did not match');
...
function replaceFunction(name, nextName, replacement) {
  ...
  if (!pattern.test(source)) throw new Error('Function baseline did not match: ' + name);
...
replaceFunction(
  'reopenDeferred',
  'verify',
  String.raw`async function reopenDeferred(taskId) {
...
replaceFunction(
  'verifyTask',
  'sync',
  String.raw`async function verifyTask(taskId) {
...
replaceFunction(
  'sync',
  'writeActiveState',
  String.raw`async function sync() {
...
replaceFunction(
  'writeActiveState',
  'activate',
  String.raw`async function writeActiveState() {
...
replaceFunction(
  'activate',
  'close',
  String.raw`async function activate(taskId, additionalAllowedPaths = []) {
...
replaceFunction(
  ...
  String.raw`async function close() {
  const ciStatus = process.argv.find((value) => value.startsWith('--ci='))?.slice(5);
  const commit = process.argv.find((value) => value.startsWith('--commit='))?.slice(9);
...
replaceFunction(
  'advanceImplementation',
  'main',
  String.raw`async function advanceImplementation() {
  const ciStatus = process.argv.find((value) => value.startsWith('--ci='))?.slice(5);
  const commit = process.argv.find((value) => value.startsWith('--commit='))?.slice(9);
...
if (source.includes('writeFile(')) throw new Error('Direct taskctl writeFile calls remain');
await writeFile(file, source, 'utf8');

const policyTest = String.raw`import { readFile } from 'node:fs/promises';
...
`;
await writeFile('tests/unit/taskctl-transaction-policy.test.ts', policyTest, 'utf8');
console.log('taskctl atomic migration generated.');
