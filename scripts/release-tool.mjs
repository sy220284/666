...
export function parseReleaseVersion(value) {
  if (
    ...
    throw new Error(
      "Release version must be a non-empty SemVer value without surrounding spaces",
    );
  }
  const match =
  ...
  if (!match)
    throw new Error("Release version must use SemVer without a leading v");
...
export function validateReleaseConfiguration({
  ...
  for (const [name, expected] of Object.entries(expectedScripts)) {
    if (packageJson.scripts?.[name] !== expected) {
      errors.push(
        "package.json must define " + name + ' as "' + expected + '"',
      );
    }
  }
...
export function evaluateReleaseGate({
    ...
    if (missingFromHold.length > 0 || extraInHold.length > 0) {
      errors.push(
        "verificationHold.verifiedTasks must exactly match TASK_INDEX" +
          (missingFromHold.length > 0
            ? `; missing ${missingFromHold.join(", ")}`
            : "") +
  ...
  if (!validCommitReference(lastVerified?.evidenceHead)) {
    errors.push(
      "lastVerifiedTask.evidenceHead must reference a committed revision",
    );
  ...
  async function visit(relativeDirectory) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        throw new Error(
          "Release assets may not contain symbolic links: " + normalizedPath,
        );
      }
...
function isAncestor(ancestor, descendant) {
  if (!validCommitReference(ancestor) || !validCommitReference(descendant))
    return false;
  try {
...
async function checkConfiguration() {
  ...
  console.log(
    "Release tooling is configured. Publishing gate: " + gateStatus + ".",
  );
}
...
async function requireReleaseGate(requestedVersion) {
  ...
  console.log(
    `Release gate passed for v${result.version} through ${result.taskId}.`,
  );
...
async function writeChecksums(requestedVersion) {
  ...
  const outputPath = path.resolve(
    ...
    readOption(
      "--output",
      path.join(path.relative(root, assetDirectory), checksumFileName),
    ),
  );
  ...
  if (
    ...
    throw new Error(
      "Checksum output must be located inside the release asset directory",
    );
  }
