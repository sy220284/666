...
function changedFilesBetween(ancestor, descendant) {
  ...
  const output = execFileSync(
    "git",
    ["diff", "--name-only", `${ancestor}..${descendant}`],
    {
      cwd: root,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
  ...
  for (const item of items) {
    ...
    if (item.status === "PASS") {
      if (!COMMIT_PATTERN.test(item?.verifiedCommit ?? "")) {
        ...
        errors.push(
          `${id}: verifiedCommit is not reachable from the release commit`,
        );
      }
      ...
      if (!Array.isArray(item?.scope) || item.scope.length === 0) {
        ...
        const stalePaths = changedSinceVerification(
          item.verifiedCommit,
          item.scope,
          head,
        );
    ...
    if (item.status === "ACCEPTED_RISK") {
      ...
      if (
        ...
        errors.push(
          `${id}: ACCEPTED_RISK requires reason, approvedBy and expiresAt`,
        );
...
export async function validateUiAcceptanceEvidence(
  state,
  repositoryRoot = root,
) {
  const errors = [];
  for (const item of state.items ?? []) {
    ...
    for (const evidence of item.evidence ?? []) {
      if (typeof evidence !== "string" || evidence.trim().length === 0) {
        errors.push(
          `${item.id}: evidence reference must be a non-empty string`,
        );
        continue;
      }
  ...
  const errors = [
    ...evaluateUiAcceptanceState(state, {
      ...
      changedSinceVerification: (verifiedCommit, scope, releaseHead) =>
        filterScopeChanges(
          changedFilesBetween(verifiedCommit, releaseHead),
          scope,
        ),
    }),
