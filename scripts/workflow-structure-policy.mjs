...
export function validateWorkflowStructure(file, source) {
  ...
  if (file === 'pr-policy.yml') {
    ...
    if (Array.isArray(types) && types.includes('closed')) {
      errors.push(
        'pr-policy.yml: merged/closed events must not rewrite the pre-merge pr-policy status',
      );
    }
  }
  ...
  if (file === 'quality.yml') {
    ...
    if (!String(macos?.if ?? '').includes("needs.route.outputs.platform_experience == 'true'")) {
      errors.push(
        'quality.yml: macOS platform experience must be enabled by the unified risk route',
      );
    }
    ...
    if (
      ...
      errors.push(
        'quality.yml: release-audit must be risk-routed and honor the exact Draft marker',
      );
    }
    ...
    if (packageGate && !hasEveryNeed(packageGate, ['route', 'quality-core'])) {
      errors.push(
        'quality.yml: temporary package-smoke compatibility gate has invalid dependencies',
      );
    }
  ...
  if (file === 'quality-core.yml') {
    ...
    if (linuxInput?.type !== 'boolean' || linuxInput?.default !== false) {
      errors.push(
        'quality-core.yml: linux_platform_experience must be a boolean defaulting to false',
      );
    }
    ...
    const productRun = product?.steps?.find(
      (step) => step?.name === 'Run product tests with coverage',
    );
    ...
    if (
      !hasEveryNeed(workflow.jobs.quality, ['product-tests', 'reliability-tests', 'desktop-e2e'])
    ) {
  ...
  if (file === 'repository-governance.yml') {
    ...
    if (!Array.isArray(push?.paths) || push.paths.length === 0) {
      errors.push(
        'repository-governance.yml: main push validation must be scoped to ruleset authority paths',
      );
    }
  }
  ...
  if (file === 'security.yml') {
    ...
    if (!Array.isArray(needs) || required.some((name) => !needs.includes(name))) {
      errors.push(
        'security.yml: aggregate security job must need route and every security sub-job',
      );
    }
    ...
    if (!source.includes('scan-secrets.mjs --history')) {
      errors.push(
        'security.yml: scheduled/manual security must retain full-history secret scanning',
      );
    }
  ...
  if (file === 'performance.yml') {
    ...
    const matches =
      source.match(/vitest run tests\/performance --no-file-parallelism --retry=1/gu) ?? [];
    if (matches.length !== 1) {
