...
test(
  'keeps a proposal pending until the author accepts it and then exposes the ending snapshot',
  async () => {
    ...
    try {
      ...
      const seeded = await page.evaluate(async () => {
        ...
        const opened = await bridge.draft.open({ projectId, chapterId: chapter.id });
        ...
        const before = await stateProposal.readSnapshot({ projectId, chapterId: chapter.id });
      ...
      const accepted = await page.evaluate(
        async ({ projectId, chapterId, locationId }) => {
          const bridge = (
