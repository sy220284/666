...
async function harness(options: ConstructorParameters<typeof ImportExportService>[2] = {}) {
  ...
  it('previews without database writes, commits one atomic volume and exports selected immutable Versions', async () => {
    ...
    try {
      ...
      expect(
        value.workspace.readProject(value.project.projectId, (database) => ({
          volumes: Number(
            database.prepare('SELECT COUNT(*) AS count FROM volumes').get()?.count ?? 0,
          ),
          chapters: Number(
            database.prepare('SELECT COUNT(*) AS count FROM chapters').get()?.count ?? 0,
          ),
          drafts: Number(
            database.prepare('SELECT COUNT(*) AS count FROM drafts').get()?.count ?? 0,
          ),
          versions: Number(
      ...
      expect(
        (await readdir(value.exportDirectory)).filter((name) => name.includes('.tmp-')),
      ).toEqual([]);
    } finally {
