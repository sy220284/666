...
async function seedFullQuota(value: Harness): Promise<void> {
  const count = MAX_RESEARCH_PROJECT_ATTACHMENT_BYTES / MAX_RESEARCH_ATTACHMENT_BYTES;
...
describe('M12-02 research closure', () => {
  it('covers the complete note lifecycle', async () => {
    ...
    try {
      ...
      expect(note.sourceType).toBe('archive');
      expect(note.sourceLabel).toBe('县志卷三');
      ...
      expect(preview.text).toBe('安全预览正文');
  ...
  it('rejects unsafe and tampered attachments', async () => {
    const value = await harness();
    try {
  ...
  it('enforces the project attachment quota', async () => {
    const value = await harness();
    try {
      await seedFullQuota(value);
      const source = path.join(value.root, 'over.txt');
  ...
  it('supports volume links and rejects cross-project targets', async () => {
    const value = await harness();
    try {
      ...
      const filtered = value.research.list({
        projectId: value.project.projectId,
