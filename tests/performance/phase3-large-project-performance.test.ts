...
async function seedLargeProject(harness: Harness): Promise<{
  ...
  await harness.workspace.writeProject(randomUUID(), project.projectId, (connection) => {
    ...
    const attachDraft = connection.prepare('UPDATE chapters SET active_draft_id = ? WHERE id = ?');
    ...
    for (let volumeIndex = 0; volumeIndex < VOLUME_COUNT; volumeIndex += 1) {
      ...
      for (let localIndex = 0; localIndex < CHAPTERS_PER_VOLUME; localIndex += 1) {
        ...
        insertChapter.run(chapterId, volumeId, `第${chapterIndex + 1}章`, (localIndex + 1) * 1024);
...
describe('Phase 3 realistic large-project performance', () => {
  it('keeps a 500-chapter 1.5M-character project within existing blocking budgets', async () => {
    ...
    try {
      ...
      expect(structure.volumes.reduce((total, volume) => total + volume.chapters.length, 0)).toBe(
        CHAPTER_COUNT,
      );
