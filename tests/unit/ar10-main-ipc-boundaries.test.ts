...
describe('AR-10 Main IPC boundaries', () => {
  it(
    'keeps the root registrar limited to domain composition and deterministic disposal',
    async () => {
  ...
  it(
    'centralizes trust, schema failure, unexpected exception and query semantics in one guard',
    async () => {
  ...
  it(
    'assigns every IPC channel reference to exactly one domain except task connect disposal',
    async () => {
