...
describe("统一风险矩阵", () => {
  ...
  it("治理文档维护保持轻量但进入Meta-Governance", () => {
    expect(
      riskPlan(["docs/process/CI_WORKFLOW_ARCHITECTURE.md"]),
    ).toMatchObject({
      fullSuite: false,
      dependencyAudit: false,
...
describe("Active文档一致性", () => {
  ...
  it("两分支旧文档会被识别", () => {
    expect(
      ...
      "CI documentation still declares the retired two-branch inventory",
    );
  });
});
...
describe("Meta-Governance权威链", () => {
  const sources = {
    ...
    riskPolicy:
      "risk-matrix.json export function riskPlan 'dependency-audit' 'windows-ime'",
  ...
  it("Release丢失UI gate时阻断", () => {
    expect(
      ...
      "Release is missing authority marker: node scripts/ui-acceptance-gate.mjs",
    );
  });
});
