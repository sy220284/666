...
const taskIndex = (entries: readonly TaskIndexEntry[]) =>
  ...
  entries
    .map(
      (entry) =>
        `| ${entry.id} | [任务](${entry.id}.md) | ${entry.dependency} | ${entry.status} |`,
    )
    .join("\n") +
  "\n";
...
function activeTaskState(overrides: ActiveTaskOverrides = {}) {
  ...
  return {
    ...
    verificationHold: {
      ...
      verifiedTasks:
        overrides.holdVerifiedTasks ?? verifiedTasks.map((task) => task.id),
      finalTask: overrides.finalTask ?? true,
      nextTaskId:
        overrides.nextTaskId === undefined ? null : overrides.nextTaskId,
    },
  };
}
...
describe("release tool", () => {
  ...
  it("creates deterministic SHA-256 entries for nested assets", async () => {
    ...
    await writeFile(
      path.join(directory, "linux", "worldforge.AppImage"),
      "alpha",
      "utf8",
    );
