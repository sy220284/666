...
describe('UI acceptance gate', () => {
  ...
  it('matches exact and recursive UI freshness scopes', () => {
    expect(scopeMatches('apps/desktop/renderer/src/App.tsx', 'apps/desktop/renderer/**')).toBe(true);
    expect(scopeMatches('scripts/ui-acceptance-gate.mjs', 'scripts/ui-acceptance-gate.mjs')).toBe(
      true,
    );
    expect(scopeMatches('packages/core-service/src/core.ts', 'apps/desktop/renderer/**')).toBe(false);
    expect(
      filterScopeChanges(
