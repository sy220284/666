...
describe("UI acceptance gate", () => {
  ...
  it("blocks unreachable verification commits", () => {
    expect(
      ...
      "CHN-TERM-001: verifiedCommit is not reachable from the release commit",
    );
  });
  ...
  it("matches exact and recursive UI freshness scopes", () => {
    ...
    expect(
      scopeMatches(
        "packages/core-service/src/core.ts",
        "apps/desktop/renderer/**",
      ),
    ).toBe(false);
    expect(
  ...
  it("validates repository evidence paths while allowing typed external references", async () => {
    const directory = await mkdtemp(
      path.join(tmpdir(), "worldforge-ui-acceptance-"),
    );
    ...
    await expect(
      validateUiAcceptanceEvidence(state, directory),
    ).resolves.toEqual([]);
